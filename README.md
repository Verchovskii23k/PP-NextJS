...
| Роутер | Процедура | Доступ | Описание |
|--------|-----------|--------|----------|
| `schedule` | `getSchedule` | `protected` | Получить расписание с фильтрами (публичное API) |
| | `filters` | `protected` | Получить доступные фильтры для расписания |
...
| | `optimizeSchedule` | `admin` | Запустить оптимизацию (имитация отжига) |
...
| `scheduleOptimizer` | `optimizeSchedule`* | `admin `| Запустить оптимизацию (имитация отжига). При включённом чекбоксе «Использовать буфер» буферные занятия автоматически размещаются в расписании (с возможностью вытеснения конфликтующих)|
