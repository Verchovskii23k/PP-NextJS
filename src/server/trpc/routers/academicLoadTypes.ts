...
export const academicLoadTypesRouter = router({
  ...
  create: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => ctx.db.insert(academicLoadTypes).values(input).returning()),
  ...
  delete: adminProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ ctx, input }) => ctx.db.delete(academicLoadTypes).where(eq(academicLoadTypes.id, input.id))),
});