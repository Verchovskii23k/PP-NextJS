...
export const unitsRouter = router({
  list: adminProcedure.query(async ({ ctx }) => ctx.db.select().from(units)),
  get: adminProcedure.input(z.object({ id: z.number() }))
  ...
  delete: adminProcedure.input(z.object({ id: z.number() }))
    .mutation(async ({ ctx, input }) => ctx.db.delete(units).where(eq(units.id, input.id))),
});